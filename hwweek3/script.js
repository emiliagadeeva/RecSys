...
window.onload = async function() {
    try {
        ...
        // Update UI status
        updateResult('Loading movie data...', 'info');
        ...
        updateResult('Data loaded successfully! Training model...', 'success');
        await trainModel();
 ...
 * This implements the core collaborative filtering approach using embedding layers
 */
function createModel(numUsers, numMovies, latentDim = 20) {
    ...
    // Input layers - these will receive user and movie IDs
    const userInput = tf.input({shape: [1], name: 'user_input'});
    const movieInput = tf.input({shape: [1], name: 'movie_input'});
    
    // Embedding layers - these learn latent factors for users and movies
    // Each user/movie gets a dense vector representation in latent space
    ...
    // Bias terms - account for user/movie specific tendencies
    ...
    // Dot product of user and movie embeddings - measures compatibility
    // Then add bias terms to account for individual tendencies
    ...
    // Flatten to get final rating prediction
...
async function predictRating() {
    ...
    try {
        ...
        updateResult(`
            ...
            <small>Latent factor model prediction based on collaborative filtering</small>
        `, 'success');
...
function populateUserDropdown() {
    ...
    // Add some sample users (first 100 to avoid overwhelming the dropdown)
